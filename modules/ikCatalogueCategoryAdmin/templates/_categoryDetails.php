  ...
  <div id="category-details">
    <ul class="sf_admin_td_actions" id="category-details-actions">
      <li class="sf_admin_action_edit">
        <?php echo link_to(__('Edit', array(), 'sf_admin'), '@catalogue_category_admin_edit?id='.$category['id']) ?>
      </li>
      <li class="sf_admin_action_delete">
        <?php echo link_to(
          __('Delete', array(), 'sf_admin'),
          '@catalogue_category_admin_delete?id='.$category['id'], $category,
      ...
      <?php if (sfConfig::get('app_catalogue_use_properties')): ?>
        <li class="sf_admin_action_edititemproperties">
          <?php echo link_to(__('Редактировать набор свойств', array(), 'messages'), url_for('@catalogue_category_admin').'/ListEditCategoryPropertySets?id='.$category['id'], array()) ?>
        </li>
      <?php endif ?>
    </ul>
    <?php if ($categoryPath): ?>
      ...
      <?php foreach ($categoryPath as $pathItem): ?>
        <a href="<?php echo url_for('@catalogue_item_admin') ?>?page=1&category=<?php echo $pathItem['id'] ?>"><?php echo $pathItem['name'] ?></a>&nbsp;&gt;&nbsp;
      <?php endforeach ?>
      </span>
    <?php endif ?>
